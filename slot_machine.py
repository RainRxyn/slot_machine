...
#starting variables that might change over time
MAX_LINES = 3
MAX_BET = 100
MIN_BET = 1

#variables that could change in time
ROWS = 3
COLS = 3
...
def get_slotmachines_spin(rows, cols, symbols):
    all_symbols = []
...
def get_bet():
    while True:
        amount_bet = input("What would you like to bet on each line?" + str(MIN_BET) + "-" + str(MAX_BET) + "$")
        if amount_bet.isdigit():
            amount_bet = int(amount_bet)
            if MIN_BET <= amount_bet <= MAX_BET:
                break
            else:
                print(f"Enter a value above ${MIN_BET} and below ${MAX_BET}")
        else:
            print("Please enter a number")
    return amount_bet
...
def spin(balance):
    ...
    slots = get_slotmachines_spin(ROWS, COLS, symbol_count)
    print_slot_machine(slots)
    winnings, winning_lines = check_winnings(slots, lines, bet, symbol_value)
    print(f"You won {winnings}")
    print(f"Your winning line(s) are", *winning_lines)
